...
function calculatePValue(
  ...
  const standardError = Math.sqrt(sumSquaredResiduals / (n - 2)) / Math.sqrt(sumSquaredX);
  ...
  if (tStat > 1.645) return 0.10; // p < 0.10
  return 0.20; // p > 0.10
}
 ...
 */
export function analyzeTemperatureTrend(data: ClimateTrendData[]): TrendAnalysis {
  if (data.length < 10) {
    return {
      slope: 0,
      intercept: 0,
      ...
      percentChange: 0,
      interpretation: 'Insufficient data for trend analysis (minimum 10 years required)',
    };
  }
  ...
  if (isSignificant) {
    ...
    interpretation = `Statistically significant ${direction} trend of ${rate}°C per year (p < ${pValue.toFixed(2)}). `;
    interpretation += `Total change: ${percentChange > 0 ? '+' : ''}${percentChange.toFixed(2)}% over ${lastYear - firstYear} years.`;
  } else {
    interpretation = `No statistically significant trend detected (p = ${pValue.toFixed(2)}).`;
  }
 ...
 */
export function analyzePrecipitationTrend(data: ClimateTrendData[]): TrendAnalysis {
  if (data.length < 10) {
    return {
      slope: 0,
      intercept: 0,
  ...
  if (isSignificant) {
    ...
    interpretation = `Statistically significant ${direction} trend of ${rate} mm per year (p < ${pValue.toFixed(2)}). `;
    interpretation += `Total change: ${percentChange > 0 ? '+' : ''}${percentChange.toFixed(2)}% over ${lastYear - firstYear} years.`;
  } else {
    interpretation = `No statistically significant trend detected (p = ${pValue.toFixed(2)}).`;
  }
...
export function analyzeExtremeEvents(
  ...
  const extremePrecipitationDays = precipitationData.filter(p => p > precip95).length;
...
export function detectChangePoints(data: ClimateTrendData[]): number[] {
  ...
  for (let i = windowSize; i < data.length - windowSize; i++) {
    ...
    const meanBefore = before.reduce((sum, d) => sum + d.value, 0) / before.length;
    const meanAfter = after.reduce((sum, d) => sum + d.value, 0) / after.length;
    
    // If difference is significant (> 1 standard deviation)
    const stdDev = Math.sqrt(
      data.reduce((sum, d) => sum + Math.pow(d.value - meanBefore, 2), 0) / data.length
    );
